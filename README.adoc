...
This program (named `gantt`) is based on link:https://github.com/plantuml/plantuml[PlantUML] v1.2021.14. +
...
automatically update pictures. Second process – `web server` which is expose _PNG/SVG_ files as static resources. All other
filetypes are filtered and never exposed via web.

[source,bash]
----
java -Duser.language=ru -Djava.awt.headless=true -jar target/gantt-0.1.3-standalone.jar -f svg -i /Users/mike/projects/planning -o :input-folder --delete-puml server  -s 0.0.0.0
